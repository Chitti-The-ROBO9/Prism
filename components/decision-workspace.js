...
export function DecisionWorkspace({ analysis, source, onNewDecision }) {
  ...
  return <motion.section className="workspace" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    <header className="workspace-head"><div><p className="eyebrow">Decision workspace <span className="demo-pill">{source === "demo" ? "Preview data" : "Interactive model"}</span></p><h1>{analysis.title}</h1><p>{analysis.summary}</p></div><button className="secondary-button" onClick={onNewDecision}><Plus size={16} /> New decision</button></header>
    ...
    <footer className="workspace-footer"><PrismMark /><div><strong>Prism does not replace your judgment.</strong><span>It helps you see what your decision depends on.</span></div><button className="secondary-button" onClick={onNewDecision}>Explore another decision <ArrowRight size={16} /></button></footer>
  </motion.section>;
}

function BlueprintPanel({ analysis, activeBlueprint, setActiveBlueprint }) {
  return <section id="blueprint" className="panel blueprint-panel"><SectionHeading eyebrow="01 — Decision blueprint" title="The anatomy of this decision." copy="Select any dimension to inspect it. These are working hypotheses, not facts." />
    <div className="blueprint-layout"><div className="blueprint-core"><div className="core-orbit"><PrismMark /></div><strong>Your decision</strong><small>10 connected dimensions</small></div>
      <div className="blueprint-grid" role="tablist" aria-label="Decision dimensions">{Object.entries(analysis.blueprint).map(([key, values], index) => <button key={key} role="tab" aria-selected={activeBlueprint === key} className={`blue-node ${activeBlueprint === key ? "selected" : ""}`} style={{ "--delay": `${index * 0.03}s` }} onClick={() => setActiveBlueprint(key)}><BlueprintIcon className={key} /><span>{blueprintLabels[key]}</span><b>{values.length}</b></button>)}</div>
      <motion.div className="blueprint-detail" key={activeBlueprint} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}><div><p className="eyebrow">{blueprintLabels[activeBlueprint]}</p><h3>{activeBlueprint === "assumptions" ? "What this decision appears to rely on" : "Current working model"}</h3></div>{analysis.blueprint[activeBlueprint].map((item, index) => <div className="detail-item" key={item}><span>{String(index + 1).padStart(2, "0")}</span>{item}</div>)}</motion.div>
    </div></section>;
}

function LensPanel({ analysis, activeLens, setActiveLens }) {
  const lens = analysis.lenses[activeLens];
  return <section id="lenses" className="panel"><SectionHeading eyebrow="02 — Reasoning lenses" title="Perspectives selected for this decision." copy="Prism chose these lenses because each adds a distinct way of examining your decision." />
    <div className="lens-layout"><div className="lens-list" role="tablist" aria-label="Reasoning lenses">{analysis.lenses.map((item, index) => <button className={activeLens === index ? "active" : ""} aria-selected={activeLens === index} role="tab" onClick={() => setActiveLens(index)} key={item.name}><span>{String(index + 1).padStart(2, "0")}</span><strong>{item.name}</strong><ChevronRight size={16} /></button>)}</div>
      <motion.article className="lens-detail" key={activeLens} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }}><div className="lens-title"><div className="lens-icon"><BrainCircuit size={22} /></div><div><p className="eyebrow">Selected because</p><h3>{lens.name}</h3></div></div><p className="why">{lens.why}</p><div className="lens-insights"><Insight label="Observation" text={lens.observation} icon={<ScanSearch />} /><Insight label="Trade-off" text={lens.tradeoff} icon={<ArrowRight />} /><Insight label="Question to sit with" text={lens.question} icon={<CircleHelp />} /><Insight label="Possible blind spot" text={lens.blindSpot} icon={<TriangleAlert />} /></div></motion.article>
    </div></section>;
}

function ScenarioPanel({ analysis, activeScenario, setActiveScenario }) {
  const scenario = analysis.scenarios[activeScenario];
  return <section id="scenarios" className="panel"><SectionHeading eyebrow="03 — Scenario explorer" title="Explore what changes when conditions do." copy="These are plausible branches—not predictions. Start with the condition, then inspect what it would change." />
    <div className="scenario-tabs" role="tablist" aria-label="Possible scenarios">{analysis.scenarios.map((item, index) => <button key={item.name} role="tab" aria-selected={activeScenario === index} className={activeScenario === index ? "active" : ""} onClick={() => setActiveScenario(index)}><span>{String.fromCharCode(65 + index)}</span>{item.name}</button>)}</div>
    <motion.div className="scenario-card" key={activeScenario} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}><div className="scenario-line"><i /><i /><i /></div><div><p className="eyebrow">Condition</p><h3>{scenario.condition}</h3></div><div className="scenario-copy"><div><p className="eyebrow">What this might change</p><p>{scenario.outlook}</p></div><div><p className="eyebrow">Worth watching</p><p>{scenario.watch}</p></div></div></motion.div>
  </section>;
}
...
function StressTest({ analysis, activeAssumption, setActiveAssumption, stress, setStress, selectedAssumption }) {
  return <article className="panel stress-panel"><p className="eyebrow">04 — Decision stress test</p><h2>Break one assumption<br />before reality does.</h2><p>Explore how robust your plan is when a key condition changes.</p>
    <div className="assumption-picker" role="group" aria-label="Assumptions to stress test">{analysis.blueprint.assumptions.map((assumption, index) => <button className={activeAssumption === index ? "active" : ""} aria-pressed={activeAssumption === index} onClick={() => { setActiveAssumption(index); setStress(false); }} key={assumption}>{assumption}</button>)}</div>
    <button className="dark-button" onClick={() => setStress(true)}><FlaskConical size={17} /> Stress test this assumption</button>
    <AnimatePresence>{stress && <motion.div className="stress-result" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} role="status"><ShieldAlert size={19} /><div><strong>Model under pressure</strong><p>If “{selectedAssumption}” weakens, Scenario B becomes more relevant. Create a two-week test before treating this path as robust.</p></div></motion.div>}</AnimatePresence>
  </article>;
}
...
function ReflectionPanel({ analysis }) {
  return <section id="reflection" className="reflection"><div className="reflection-intro"><p className="eyebrow">06 — Reflection, not recommendation</p><h2>Leave with better<br /><span>questions.</span></h2><p>Prism has not chosen for you. It has made the decision more visible so you can move forward deliberately.</p></div>
